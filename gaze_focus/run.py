...
def run(cameras=(0,), trigger="blink", dwell=0.4, idle=0.6, cooldown=None,
        smooth=0.25, dry_run=False, verbose=False, overlay=False):
    if cooldown is None:
    ...
    print(f"gaze-focus running ({'dry-run; ' if dry_run else ''}{how}); Ctrl-C to stop.")
    ...
    try:
        while True:
            ...
            time.sleep(0.015)
            feats_list, blink_score, stamp = ext.latest()
            now = time.time()
            if x11.any_key_down():
                last_key = now
